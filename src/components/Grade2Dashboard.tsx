...
import { GRADE2_STORIES } from "../data/grade2Data";
import {
  Sparkles,
  BookOpen,
  Scale,
...
interface Grade2DashboardProps {
  userStats: UserStats;
  onSelectStory: (story: IslandStory) => void;
  onOpenWordGame: (mode: "synonym" | "antonym") => void;
}

export const Grade2Dashboard: React.FC<Grade2DashboardProps> = ({
  userStats,
  onSelectStory,
  onOpenWordGame,
}) => {
  ...
  const [viewMode, setViewMode] = useState<"stories" | "wordGames">("stories");

  const completedCount = userStats.completedLevels.filter((id) =>
    GRADE2_STORIES.some((s) => s.id === id)
  ).length;

  const filteredStories = GRADE2_STORIES.filter((story) => {
    const matchesSearch =
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      {/* Grade 2 Hero Banner */}
      <div className="bg-gradient-to-r from-blue-600 via-teal-600 to-emerald-700 rounded-3xl p-6 sm:p-8 text-white shadow-xl relative overflow-hidden border-4 border-blue-400">
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-500/30 rounded-full text-xs font-black tracking-wider uppercase mb-2 border border-blue-300/30">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 flex items-center gap-6 text-center">
            ...
            <div className="border-l border-white/20 pl-6">
              <div className="text-xs font-bold text-blue-200">Kazanılan Altın</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <button
          ...
          className={`p-6 rounded-3xl border-3 text-left transition-all relative group shadow-md ${
            viewMode === "stories"
              ? "bg-white border-blue-500 ring-4 ring-blue-100 scale-102"
        ...
        >
          ...
          <div className="font-black text-lg text-slate-900">Zıt Anlamlı Kelimeler</div>
          <div className="text-xs font-bold text-slate-500 mt-1">
            Büyük-Küçük, Sıcak-Soğuk gibi 50+ zıt kelime eşleştirme
          </div>
          <div className="mt-4 flex items-center justify-between">
        ...
        >
          ...
          <div className="font-black text-lg text-slate-900">Eş Anlamlı Kelimeler</div>
          <div className="text-xs font-bold text-slate-500 mt-1">
            Okul-Mektep, Hediye-Armağan gibi 50+ eş anlam eşleştirme
          </div>
          <div className="mt-4 flex items-center justify-between">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              placeholder="Metin veya kahraman ara..."
              className="w-full pl-10 pr-4 py-2.5 rounded-2xl border-2 border-slate-200 focus:border-blue-500 outline-none text-xs font-bold text-slate-800"
            />
          </div>
        </div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <h3 className="font-black text-slate-900 text-base group-hover:text-blue-600 transition-colors line-clamp-1">
                    {story.title}
                  </h3>
